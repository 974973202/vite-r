 ...
 * @param sheetNames excel工作表名
 * @param columns antd columns
 * @param dataSource antd dataSource
...
export const exportXLSX = ({
  ...
  for (const [sheetIndex, sheetName] of sheetNames.entries()) {
    const _columns = sameType(columns[sheetIndex], 'Array')
      ? columns[sheetIndex]
      : columns;
    const _dataSource = sameType(dataSource[sheetIndex], 'Array')
...
const getHeaderData = ({
  ...
  const headerArr: HeaderCellType[][] = getHeader2dArray({
    columns,
    headerLevel,
  });
